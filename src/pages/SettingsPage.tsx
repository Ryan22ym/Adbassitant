...
const VERSION_NOTES: Record<string, string> = {
  '1.0.31': '本版本让在线更新支持「跨版本」了。以前在线更新的小包只对紧挨着的上一个版本有效 —— 它内部记着一份运行库指纹，必须跟你机器上的完全一致才肯装。所以只要你漏更了几版（比如还停在 1.0.2，线上已经 1.0.35），点更新就会被判「运行库不一致」直接拒收，只能自己去下八十多兆的完整安装包。现在改成：更新源会把同一个新版本针对若干个旧版本各备一份小包，客户端先按你机器上的实际指纹挑最合适的那一份（挑中的那份通常只有几百 KB）；要是你漏更的版本太多、一份都对不上，它会自动改走「完整资源包」—— 那个包里带着全部随程序发布的资源文件，因此不挑版本，跨多少个版本都能一次升到位。界面上会提前告诉你会下哪一个、大概多大，不会让你先下完几十兆才被拒绝。同时顺带解决了「新版本需要新增资源文件」的问题：更新包里的新文件所在目录如果本机还没有，替换前会自动建出来（失败回滚时也会把多建的空目录收掉，不在你的安装目录里留垃圾）。',
  '1.0.30': '本版本修的是上一版弱网模拟里一个会让它「点了没用」的问题。症状是：在电脑上点了「启动弱网模拟」，界面提示已经生效，但手机上的 VPN 压根没起来（通知栏和系统设置里都看不到），弱网自然一点效果也没有；个别时候它能闪一下然后立刻消失。原因是手机建立这条 VPN 通道并不是按一下就好的：电脑把启动指令发到手机之后，手机还得花一秒上下申请通道、配好路由，中间有一小段真空期。而电脑端在发完指令后立刻就去问了一句「现在跑起来了吗」，得到的回答当然还是「还没有」，于是被当成「手机上已经停了」，反手就把刚建好的通道关掉了 —— 通道刚建好就被自己关掉，表现出来就是没开出、或者一闪即逝。这一版改成：指令发下去之后会等手机确认通道真的建好了才算启动成功（这段时间界面上显示「正在等待设备侧建立隧道」），并且启动后的头几秒里不再把「还没起来」误判成「已停止」；万一手机那边一直起不来（比如配套 App 被系统冻结了），也会明确报出来，而不是静悄悄地像没点一样。另外顺手修了一个误报：弱网正在生效时，界面下方的设备能力检查会显示「尚未授权 VPN」，其实早就授权过了 —— 这个提示会让人以为还得再去授权一次，现在只在真的没授权时才出现。',
  '1.0.29': '本版本把弱网模拟整个换了一套实现。以前是给手机设一个全局代理、由电脑端转发来做限速丢包，问题是它只对老实走系统代理的 App 有效 —— 大量 App（尤其是游戏和自研网络库）自己直连，根本不理会这个代理，对它们来说弱网等于没开；而且改系统代理要写系统设置，部分机型（比如 ColorOS）直接不给这个权限；程序异常退出时还会把代理配置留在手机上，表现出来就是「用完手机断网」。这一版改成在手机上装一个配套 App，用系统自带的 VPN 通道接管全部流量，在 IP 层逐个数据包做限速、丢包、延迟、抖动与乱序：App 走不走代理都一样被管到，覆盖是完整的，也不再写任何系统设置。第一次使用弱网时会自动把配套 App 装到设备上（已随程序内置，不用你另外找包），手机上确认一次 VPN 授权即可，之后参数仍然在面板上调、改完即时生效。另外做了几层保底：手动停止、程序退出、心跳超时都会关闭通道，即使程序被强制结束，网络也会随通道关闭立刻恢复，不会再出现「用完断网」。原来的代理方案作为备选保留，遇到不支持 VPN 的设备仍会自动回退。 另外修了一个更新链路上的问题：从 1.0.28 及更早版本升级时，更新包会带上下载随包的弱网 App，而它原先被放在一个旧版本里还不存在的子目录里，更新前的检查会把「目录还没建」误判成「目录不可写」，直接把整个更新包拒收。现在这个文件改放到程序本来就有的目录下，更新逻辑也会在替换前自动把缺失的目录建出来。',
  '1.0.28': '本版本把「拖进去装」时的安装方式变成了当场可改。以前多台设备同时在线时，拖入安装包会先弹出「装到哪台设备？」，那一屏只把你的安装方式显示成一个灰标签 —— 想改成清洁安装，得先取消、回页面上改好、再重新拖一次。现在那个标签换成了可点的三选一：覆盖安装（保留数据）/ 清洁安装（先卸载、清除数据）/ 全新安装（已存在则拒绝），默认仍是覆盖安装这个不破坏数据的选项；每种方式下面都会写清后果，切到清洁安装会明确提示数据会被清除。这里改的只作用于这一次安装，不会改掉你的全局默认，所以下次拖进来依然是覆盖安装。另外修了一个隐藏问题：AAB 走「多设备 → 选设备」这条路时，你在界面上选的拆包签名会被丢掉，现在会正确沿用。',
  '1.0.27': '本版本给「Logcat 导出」加上了固定的存放位置。以前每次导出都要在保存框里挑一次目录，导完还得自己找过去 —— 日志东一份西一份。现在导出目录默认固定在 D:\\adblogs 下面，程序会按「设备 → 日期」自动分层：先是机型加序列号（比如 OPPO CPH2579 75CMPR59NFRSLBR4，同型号多台也能分得清），再是当天的日期（如 2026-09-21）；目录不存在会自动创建，不用你自己先建好文件夹。导完之后程序会直接把那个目录打开，省掉「导完去找文件」这一步。想换个盘或换个位置，在面板上点「选择…」挑一个根目录即可，会记住你选的位置，随时可以「恢复默认」。另外顺手修了一个不容易发现的问题：连续导出如果赶在同一秒，以前后导出的那份会悄悄盖掉前一份，现在会自动换个文件名，两份都在。',
  '1.0.26': '本版本给「常用工具」页加了一个「Logcat 导出」：以前想看设备日志，得先到「实时 Logcat」页点开始抓取，抓了一段时间再保存 —— 拿到的是点开始之后才产生的日志。这一版补上另一半：直接把设备日志缓冲区里已有的日志一次性读出来导成文本文件，不用先开抓取，适合复现完问题之后回头把刚才那段导走。导出前可以挑日志级别（V/D/I/W/E/F 六档）、按 TAG 过滤（支持 * 通配，可一次写多个）、按关键字过滤（逗号分隔，命中任一即保留），缓冲区也能多选（main / system / crash / events）；另有「只看错误」「闪退/ANR」「Activity 启动」三个快捷预设。导出完成后会显示实际写入了多少行、过滤掉多少行、文件多大，并能直接在文件夹中定位。',
  '1.0.25': '本版本做的是「把图标统一」，功能没有任何变化。上一版给设备行的快捷动作配图标时用的是 emoji —— 每个系统画出来都不一样，和侧边导航那套细线图标也搭不上。这一版把全应用的图标收拢到一个统一图标源：24 格画布、统一线宽、颜色跟随文字（悬停变主题色、危险动作变红都是自动带的），12 种快捷动作各配了不容易看错的图形（比如「清数据」用橡皮擦而不是垃圾桶，免得被当成卸载）。顺带把导航那套图标也并进同一个模块，以后全应用只有一份图标定义。',
  '1.0.24': '本版本在设备列表每台设备右侧加了一组「快捷动作」，常用的三个操作不用再切页面：清数据（清掉当前前台应用的本地数据）、桌面重进（回桌面再重新打开，不杀进程）、杀进程重进（彻底结束再打开）。行内直接显示两个，其余收在闪电按钮里，菜单顶部还会标出当前前台是哪个应用。这套动作可以自己配：内置 12 种（清数据 / 桌面重进 / 杀进程重进 / 清数据重进 / 强制停止 / 启动 / 截图到电脑 / 回桌面 / 返回键 / 亮屏 / 息屏 / 自定义命令），能改名、能排序、能设「执行前确认」，作用对象既可以是「当前前台应用」，也可以钉死成某个包名，还能直接写 shell 命令（支持 {pkg} / {serial} 占位）。行内最多放 3 个（位置有限，超出的自动收进菜单），配置存在本机，随时可一键恢复默认。',
  '1.0.23': '本版本做的是「上线」这一步：把在线更新的默认更新源填成了真实地址（服务器端就是一个静态目录，里面放着 latest.json 与更新包）。装上这一版之后你不需要做任何设置 —— 程序启动约 8 秒会自己安静地查一次，发现有新版本时左侧「设置」旁会冒一个小红点，进去点「检查更新」就能看到版本号、更新说明与发布时间，确认后点「下载并更新」即可，下载完自动校验、校验不过就地删除。除此之外没有别的改动：离线那条「选择更新包…」照旧可用；「更新源」也仍然可以在设置页里自己改成别的地址，你手动填的优先级更高。',
