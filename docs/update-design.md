...
> 🔴 **产物形态已变（v1.0.24 起）**：打包只出 NSIS 安装包，**不再产出便携包**；
> 更新也只发**安装版增量小包**。本文中「便携版增量 = 整包替换 exe（约 84 MB）」
> 属于历史分析，相关代码路径（`PORTABLE_EXECUTABLE_FILE` 识别、`kind: 'portable'`）
> 仍保留在 `update.ts` / `update-core.ts` 里，只是不再有新版本用到。
> 结论看 `docs/online-update-rollout.md`（操作方案）与 `docs/online-update-design.md`（设计）。
...
- 目标目录不可写 → 提前报错（只读目录/U盘写保护）
  - ⚠️ 必须把「不可写」与「**还不存在**」分开：包里可能出现本版才新增的文件，
    它所在的目录在本机压根没有（v1.0.29 的随包弱网 APK 就是）。
    这时要沿父目录上溯探写权限，并由助手在替换前 `New-Item -Force` 建出来。
    把二者混为一谈的后果很隐蔽 —— 报错说「目录不可写」，用户去查权限什么也查不出来，
    而实际上这是一次完全合法的更新（v1.0.29 首版就栽在这里，见 §6）
...
| 包里带本版新增的文件（落在本机还没有的目录） | 预检沿父目录上溯判可写性；助手替换前 `New-Item -Force` 建目录。**更新包里的路径要始终落在已有目录下**（随包 APK 因此放 `bin/` 根下而不是 `bin/weaknet/`） |
