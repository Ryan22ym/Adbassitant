...
> 🔴 **当前策略：托管上保留最近 10 个版本（v1.0.26 起执行）。**
> 每个版本 = `patch.zip` + `.sha256`；`latest.json` 只有 1 个、永远指向最新版。
> **只有超过 10 个版本时才清理最老的那个** —— 平时发版只管上传，不用删任何东西。
> 好处：回退到 10 版以内的任意版本时，**直接改 `latest.json` 的 `packages` 指向那一版的包即可**（文件还在），
> 不需要在本地重新生成再传。
> ⚠️ 这取代了 v1.0.25 时执行过的「只保留最新一版」——当时已把 ≤v1.0.24 的包删掉，**那批版本永久不可回退**。
...
| 层次 | 场景 | 动作 | 影响 |
|---|---|---|---|
| **① 清单回退** | 新版有严重 bug，不想再有人升上去 | 把 `latest.json` 的 `packages` 指回**仍在托管上的那版包**（最近 10 版内，文件还在，改 version / url / size / sha256 即可）。超过 10 版已被清理的就没了 → 只能「清空 `packages`」或不管 | 已升级的用户**不会自动降级**（客户端只认「比我新」）；未升级的用户回到旧版本 |
