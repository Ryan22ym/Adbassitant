...
> 🔴 **我们的现状走的就是这条路（CloudBase 静态托管），跟 Lighthouse 那台服务器无关。**
> - 环境：`buddybase-d8g4m4rz6306e4648`（别名 `buddybase`，上海）
> - 控制台路径：腾讯云 → 云开发 CloudBase → 环境 `buddybase` → **静态网站托管 → 文件管理 → `adb-assistant/`**
> - 入口：`https://buddybase-d8g4m4rz6306e4648-1485935404.tcloudbaseapp.com/adb-assistant/`
> - 上传方式：CloudBase MCP 直传（`manageHosting upload`），**文件不落任何服务器**
> - 别拿服务器 IP / Lighthouse 面板去找包 —— 找不到是正常的。
...
https://<你的域名>/adb-assistant/
  ...
  ADB桌面助手-v1.0.25-patch.zip                    ← 安装版增量包（约 150–260 KB）
  ADB桌面助手-v1.0.25-patch.zip.sha256             ← 可选：给人看的，客户端不读
```

> 🔴 **当前策略：托管上只保留最新一版的更新资源（v1.0.25 起执行）。**
> 历史版本的 `patch.zip` 与 `runtime-vX.json` 一律删除 —— 客户端**不会降级**，
> 回退靠的是 `latest.json`（§6 层①）和客户端本地备份（§6 层③），都不需要服务器上的旧包。
> 代价：**不能再把 `latest.json` 指回某个已删的旧版本**（文件没了，下载会 404）。
> 真要那种回退，得先在本地重新生成那一版的包再传上去。
...
步骤 6 只证明「清单读得到」，**不证明「包是对的」**。这一步纯 HTTP 就能做，不需要登控制台：

```bash
# ① 拉清单，记下 Content-Type / Cache-Control / 声明的 size+sha256
curl.exe -s https://<你的域名>/adb-assistant/latest.json

# ② 把每个包整包下回来算 sha256，与清单逐项比对
python e2e-tmp/_verify-remote.py
```

必须同时满足：

- 两个包的 **`size` 与 `sha256` 都与清单一致** —— 这才叫「清单说啥、包就是啥」
- `latest.json` 的 `Cache-Control` 是 `no-store` / `no-cache`；否则 CDN 缓存住旧清单，
  **你更新了服务器、客户端也看不见**
- `Range: bytes=0-15` 探一下包，`Content-Type` 应为 `application/zip`
- 随手探一个不存在的路径，确认 404 兜底正常（而不是 200 返回错误页）

**最终验收仍然是真机**：`e2e-tmp/_e2e-online-update.cjs` 的 `BASE` 默认就指向真实托管地址，
跑一遍应看到 `检查状态=available → 下载校验 → replaced → health ok → 45 秒不回滚`，
且落地 `app.asar` 的 sha256 与本地产物**逐字节相同**。这才是「真上线了」的证据。

#### 🔴 两条铁律（都是踩出来的）

**① 「服务器上那份是哪个构建」要比 sha256，不能比版本号。**
同一个版本号可以对应多个不同内容的构建（改完 bug 重新打包、版本号没动）—— 线上躺着旧包、
本地是新包，是「修了但没生效」的头号原因。把**装机版 / 远端包 / 本地各 `out-*` 的 `resources/app.asar`**
一起算 sha256，谁和谁相等一目了然。

**② 判断「某个构建里有没有某修复」，不要在 asar 二进制里裸搜关键词。**
asar 是未压缩容器、正文可搜，但假阳性极常见：搜 `SIGKILL` 在**没修**的构建里也命中 ——
代码里另有一个不相干的 `child.kill('SIGKILL')`（kill 子进程用的工具函数）。要搜更长、带上下文的串
（如 `process.kill(process.pid`），或定位 offset 把前后文打出来人眼确认。

#### 🔴 已发布的 `latest.json` + 包 = 不可变

改了内容就必须**升版本号重发**。同名覆盖会同时造成两个后果：客户端版本号相同 ⇒ **不会触发更新**
（等于没修）；而已经拿到旧包的机器**永远拿不到新内容**。

只有在能**证明「该构建从未被任何客户端成功安装过」**时才允许同名覆盖。判据：
它自己缺当前修复 ⇒ 它装不动自己；唯一能装上它的路径是「已经带修复的客户端去拉它」，
而那个客户端从未分发 ⇒ 覆盖安全。
...
| 层次 | 场景 | 动作 | 影响 |
|---|---|---|---|
| **① 清单回退** | 新版有严重 bug，不想再有人升上去 | ⚠️ 当前已删历史包，**别再改回旧版本号**（会 404）；改用「把 `latest.json` 的 `packages` 清空」或直接不管 —— 反正没人会降级 | 已升级的用户**不会自动降级**（客户端只认「比我新」）；未升级的用户看不到更新 |
...
| 更新到「最新版」了，但本次修的 bug 还在 | 线上包是**改 bug 之前的同版本构建**（版本号撞了） | 比 sha256 确认线上包到底是哪个构建；要换内容必须**升版本号**重发（§3 步骤 7） |
| 本机装了新版，行为却像旧版 | 装机 `app.asar` 与某个旧 `out-*` 逐字节相同 | 用 sha256 比对装机版与各构建，别只看版本号（§3 步骤 7） |
